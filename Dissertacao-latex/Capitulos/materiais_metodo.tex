...
Conforme mencionado anteriormente, o objetivo principal dessa dissertação foi especificar e aplicar um modelo de RNA para análise e predição de valores acionários. Neste capítulo foi realizada a pergunta de pesquisa e dos objetivos do trabalho na escolha da metodologia. Em seguida, esta metodologia foi descrita e classificada quanto ao seu conteúdo e quanto aos métodos empregados na coleta e análise de dados.
...
Para iniciar o processo de desenvolvimento deste trabalho, foi necessário definir quais ações foram escolhidas para compor a implementação. Os critérios selecionados foram caracterizados pelo alto potencial de movimentação e lucratividade na NASDAQ, ou seja, são as empresas que mais influênciam na ecônomia, que, por coincidência, são do ramo da tecnologia \cite{christie}.

Tendo em vista os critérios especificados acima, as empresas selecionadas e o código de suas ações para a realização da coleta dos dados foram:
\begin{itemize}
	\item Apple (AAPL);
	\item Amazon (AMZN);
	\item Intel (INTC);
...
Com a definição da bolsa de valores em questão e das empresas utilizadas para realizar a implementação deste trabalho, a próxima etapa se dá pela necessidade em coletar seus dados histórios.
...
Com a especificação das séries coletadas, foi necessário preparar um ambiente que dispõe o acesso à esses dados de forma simplificada. 
...
Sendo assim, foi desenvolvido um ambiente que proporcione o uso dessa ferramenta, junto à biblioteca citada na \autoref{pandas-lib} para realizar as buscas pelos dados necessários.

\subsection{PyCharm IDE}
Para auxiliar no \textit{script} de coleta de dados foi utilizada utilizada uma \textit{Integrated Development Environment} (IDE). PyCharm é uma IDE proprietária do grupo JetBrains que conta com uma versão livre para estudantes, que será realizada neste trabalho.
 
A IDE suporta o desenvolvimento com a linguagem de programação Python desde a sua versão 2.4 até a versão 3.6. Vale citar que outras IDEs para o desenvolvimento poderiam ser utilizadas, porém a praticidade e a boa documentação das ferramentas JetBrains são fatores que justificam a sua utilização \cite{pycharm}.
...
Tendo em vista os critérios estabelecidos na seção anterior, chegou-se a definição de que a RNA deverá possuir uma arquitetura de camadas alimentada adiante, um método de aprendizado supervisionado e uma função de custo, para controlar o processo de generalização da mesma.

\citeonline{haykin2000} direciona este modelo de rede ao \textit{Perceptron} de Múltiplas Camadas (MLP, \textit{Multilayer Perceptron}), que são redes neurais compostas por um conjunto de unidades sensoriais que constituem a camada de entrada, uma ou mais camadas ocultas e uma camada de saída, onde o sinal de entrada se propaga para frente através da rede, camada por camada. Redes MLP trabalham normalmente junto ao algoritmo \textit{backpropagation} para realizar seu treinamento e aprendizado.

Tendo em vista a especificação do modelo da rede e dos dados utilizados para realizar o treinamento e os testes, a próxima etapa foi definir as entradas que foram utilizadas na RNA. Como o objetivo do trabalho é realizar a predição do valor de abertura das ações em dias posteriores aos treinados, todas as váriaveis coletadas serão utilizadas como entrada da RNA. Portanto, os parâmetros que irão compor a camada de entrada da RNA são:
...
Por isso, para a grande maioria dos problemas utiliza-se apenas uma camada escondida, onde sua quantidade de neurônios é definida buscando um meio termo entre a quantidade de entradas e a sua saída, tentando evitar as complicações evidenciadas nos tópicos anteriores. De acordo com \citeonline{nielsen}, a quantidade de neurônios para a camada oculta deve ter aproximadamente 2 vezes a quantidade de variáveis de entrada. 

Mediante estas informações, o modelo desenvolvido tem apenas uma camada intermediária composta por 13 neurônios, que seguem as especificações propostas por \citeonline{nielsen}, \citeonline{haykin2000} e \citeonline{marangoni} buscando otimizar a sua estrutura e obtendo um resultado eficiente para o problema.

Por fim, a saída da rede é composta por uma única camada que contém o valor de abertura do dia seguinte. É importante deixar claro que este valor, durante o processo de treinamento, é preenchido com os valores reais das ações em relação ao período coletado. Na Figura 11 é possível visualizar a composição gráfica da arquitetura proposta.
...
A partir do modelo de rede definido, é necessário classificar os dados que serão utilizados na fase de treinamento e os que serão utilizados para a realização dos testes. Como no trabalho foram coletadas as ações de 4 empresas distintas, para cada empresa, individualmente, foram realizados os processos de treinamento e testes, com objetivo de observar o comportamento da rede nas mais diversas situações e proporcionar uma análise mais detalhada do modelo.

Os dados utilizados para o treinamento da rede devem ser significativos e cobrir amplamente o domínio do problema, e não apenas envolver os casos de operações normais ou rotineiras, mas também incorporar casos de exceção e de situações limites. Seguindo o funcionamento do algoritmo \textit{backpropagation}, os dados treinados seguiram os seguintes passos:
...
A Figura 12 demonstra, de maneira mais clara, como a biblioteca PyBrain auxilia no projeto.
...
Também é importante deixar evidente que a implementação das técnicas foram executadas em um computador pessoal com um processador Intel(R) Core(TM) i5-4200U com 2.60 \textit{gigahertz} e uma memória de acesso aleatório (RAM) DDR3 com 6 \textit{gigabytes}. 
